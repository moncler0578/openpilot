...
REVERSE_REENGAGE_MIN_SPEED = 5.0 * CV.KPH_TO_MS
...
class Controls:
  ...
  def update_reverse_reengage(self, CS):
    ...
    # Some Hyundai cars briefly drop cruise availability in R and publish a
    # new pcmEnable edge when D is selected. Hold that edge until the requested
    # 5 km/h threshold, then re-engage lateral control only.
    if CS.vEgo < REVERSE_REENGAGE_MIN_SPEED:
      if EventName.pcmEnable in self.events.events:
        self.events.events.remove(EventName.pcmEnable)
      return
  ...
  def state_control(self, CS):
    ...
    # Opt-in via LiveTorqueEnabled (default OFF -- see live_tune.live_torque_enabled(),
    # added 2026-08-18 after a real-car MDPS fault report). torqued keeps
    # learning/publishing regardless so the data is ready whenever it's turned
    # on; this gate only controls whether it's actually *applied*.
    # read_torque_params() in latcontrol_torque.py still gives priority to a
    # manual LateralTorqueCustom override whenever that's enabled.
    if self.CP.lateralTuning.which() == 'torque' and isinstance(self.LaC, LatControlTorque) and live_tune.live_torque_enabled():
    ...
    # CarrotLearning steering recommender (backport, see carrot_lat_learning.py)
